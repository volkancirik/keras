...
class ActivityRegularizer(Regularizer):
	def __init__(self, l1=0., l2=0., l_bin = 0., k = 0.25):
		self.l1 = l1
		self.l2 = l2
 		self.k = k
		self.l_bin = l_bin
	...
	def __call__(self, loss):
		...
		loss += self.l2 * T.sum(T.mean(self.layer.get_output(True) ** 2, axis=0))
 		loss += self.l_bin * T.sum(T.mean( T.pow(self.layer.get_output(True),self.k) * T.pow(1 - self.layer.get_output(True),self.k) , axis=0))
		return loss
